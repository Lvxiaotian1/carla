...
if [[ -d "${LLVM_BASENAME}-install" ]] ; then
  ...
  git clone --depth=1 -b release_80  http://github.fishros.org/https://github.com/llvm-mirror/llvm.git ${LLVM_BASENAME}-source
  git clone --depth=1 -b release_80  http://github.fishros.org/https://github.com/llvm-mirror/libcxx.git ${LLVM_BASENAME}-source/projects/libcxx
  git clone --depth=1 -b release_80  http://github.fishros.org/https://github.com/llvm-mirror/libcxxabi.git ${LLVM_BASENAME}-source/projects/libcxxabi
...
if [[ -d "${RPCLIB_BASENAME}-libcxx-install" && -d "${RPCLIB_BASENAME}-libstdcxx-install" ]] ; then
  ...
  git clone -b ${RPCLIB_PATCH} http://github.fishros.org/https://github.com/carla-simulator/rpclib.git ${RPCLIB_BASENAME}-source
...
if [[ -d "${GTEST_BASENAME}-libcxx-install" && -d "${GTEST_BASENAME}-libstdcxx-install" ]] ; then
  ...
  git clone --depth=1 -b release-${GTEST_VERSION} http://github.fishros.org/https://github.com/google/googletest.git ${GTEST_BASENAME}-source
...
if [[ -d "${RECAST_BASENAME}-install" &&
  ...
  git clone http://github.fishros.org/https://github.com/carla-simulator/recastnavigation.git ${RECAST_BASENAME}-source
...
if ${USE_CHRONO} ; then
  ...
  CHRONO_TAG=6.0.0
  # CHRONO_TAG=develop
  CHRONO_REPO=http://github.fishros.org/https://github.com/projectchrono/chrono.git
...
PATCHELF_REPO=http://github.fishros.org/https://github.com/NixOS/patchelf/archive/${PATCHELF_VERSION}.tar.gz
